...
class PostRequestsListExportingController extends Controller
{
	public function exportAction(Request $req)
	{
		...
		if (isset($param)) {
			...
			// fill data
			for ($i = 0; $i < count($postRequests); $i++, $row++) {
				$selectedBudget = explode('-', $postRequests[$i]->getSelectedBudget());
				$level = $postRequests[$i]->getLevel();
				if ($level == 1) {
					$status = $postRequests[$i]->getChairApproved();
				} else {
					$cApproved = $postRequests[$i]->getCfoApproved();
					$pApproved = $postRequests[$i]->getPresidentApproved();
					$sApproved = $postRequests[$i]->getSecretaryApproved();
				...
				$excelObj->getActiveSheet()->setCellValue("A$row", $postRequests[$i]->getRid());
				$excelObj->getActiveSheet()->setCellValue("B$row", $user_array['name'][$postRequests[$i]->getRequester()]);
				$excelObj->getActiveSheet()->setCellValue("C$row", $user_array['email'][$postRequests[$i]->getRequester()]);
				$excelObj->getActiveSheet()->setCellValue("D$row", $postRequests[$i]->getExplanation());
				$excelObj->getActiveSheet()->setCellValue("E$row", $selectedBudget[1]);
				$excelObj->getActiveSheet()->setCellValue("F$row", $postRequests[$i]->getAmount() . " " . $currency_array['code'][$postRequests[$i]->getCurtype()]);
				$excelObj->getActiveSheet()->setCellValue("G$row", ($postRequests[$i]->getActualAmount() > 0 ? $postRequests[$i]->getActualAmount() . " USD" : ""));
				
				$excelObj->getActiveSheet()->setCellValue("H$row", ($postRequests[$i]->getPreApproval() ? "Yes" : "No"));
				$excelObj->getActiveSheet()->setCellValue("I$row", sprintf("#%08d", $postRequests[$i]->getPrid()));
				$excelObj->getActiveSheet()->setCellValue("J$row", $postRequests[$i]->getNumberOfInvoices());
				
				$excelObj->getActiveSheet()->setCellValue("K$row", $statusText);
				$excelObj->getActiveSheet()->setCellValue("L$row", $postRequests[$i]->getDate()->format('m/d/Y'));
			}
