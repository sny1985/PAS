...
class PreRequestsListExportingController extends Controller
{
	public function exportAction(Request $req)
	{
		...
		if (isset($param)) {
			...
			// fill data
			for ($i = 0; $i < count($preRequests); $i++, $row++) {
				$selectedBudget = explode('-', $preRequests[$i]->getSelectedBudget());
				$level = $preRequests[$i]->getLevel();
				if ($level == 1) {
					$status = $preRequests[$i]->getChairApproved();
				} else {
					$cApproved = $preRequests[$i]->getCfoApproved();
					$pApproved = $preRequests[$i]->getPresidentApproved();
					$sApproved = $preRequests[$i]->getSecretaryApproved();
				...
				$excelObj->getActiveSheet()->setCellValue("A$row", $preRequests[$i]->getPrid());
				$excelObj->getActiveSheet()->setCellValue("B$row", $user_array['name'][$preRequests[$i]->getRequester()]);
				$excelObj->getActiveSheet()->setCellValue("C$row", $user_array['email'][$preRequests[$i]->getRequester()]);
				$excelObj->getActiveSheet()->setCellValue("D$row", $preRequests[$i]->getExplanation());
				$excelObj->getActiveSheet()->setCellValue("E$row", $selectedBudget[1]);
				$excelObj->getActiveSheet()->setCellValue("F$row", $preRequests[$i]->getAmount() . " " . $currency_array['code'][$preRequests[$i]->getCurtype()]);
				$excelObj->getActiveSheet()->setCellValue("G$row", $statusText);
				$excelObj->getActiveSheet()->setCellValue("H$row", $preRequests[$i]->getDate()->format('m/d/Y'));
			}
